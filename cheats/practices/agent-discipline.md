...
<!-- F-028 -->
A background task's returned id means **launched**. Never *finished*, never
*succeeded*. Poll it, and pull its result explicitly. Finished-and-silent is the
...
- **Strong / default effort** where the stage *is* the judgment — architecture,
...
directories sitting *outside* the obvious root — a search scoped to `src/` will
