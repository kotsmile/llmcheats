---
title: Agent I/O discipline — what one pass costs
summary: A pass is generation-bound, not read-bound — wall clock is roughly output tokens over 55 — so artifact size caps are the lever, batching reads is hygiene.
keywords: [agent pass, wall clock, output tokens, generation bound, tool round trip, batching reads, grep, read bound, vendor source, live data, artifact cap, 12KB plan, 8KB audit, 2KB hand-back, reasoning tokens, exploration budget, 25 tool calls]
related:
...
# Agent I/O discipline — what one pass costs
...
## Where a pass spends its wall clock
...
A pass is long because it generated a lot, and for almost no other reason. The
output-size section below is therefore the one that moves the number; the two
before it are hygiene worth seconds and tokens respectively. Spend your
...
**Do not expect minutes back from closing it.** A tool round trip in those
passes measured about 0.4 seconds; batching a 43-call planning pass perfectly
saves it roughly 19 seconds out of 948.

Beware the tempting arithmetic that says otherwise. Dividing a pass's duration
by its call count yields something like 26 seconds a call and makes batching look
...
**dependent** calls only — where the second call's target comes out of the first
one's output.
...
- **Dependency and vendor source** — `site-packages/`, `node_modules/`,
  ...
  internals while writing a plan. Work from the library's documented behavior; if
  the plan turns on an undocumented internal, that is an assumption to state, not
  an afternoon to spend.
- **Live data stores.** The same pass opened the production SQLite database three
  times with inline Python. Schema questions are answered by the migration files.
...
- **The test suite as a reading device.** Running a full suite to learn what the
  code does is ten minutes spent on the wrong artifact. A developer runs tests to
  verify a change; a planner or a reviewer does not run the suite at all — a
  targeted check that answers a specific question is a different thing.
...
## Sizing output to the decision it unblocks
...
- **Omit what the scope does not reach, and say what you omitted and why.** One
  ...
  omission reads as an oversight; a stated one is a scoping decision the reviewer
  can push back on.
...
- The two-sentence rule for anything addressed to a human
  (`devflow/principles.md`) survives a large artifact: the artifact is the file,
  the message about it is still two sentences.
...
rather than the agent setting it for itself (`devflow/flow-cost.md`).
...
sixteen. The reporting cadence in `devflow/flow-visibility.md` is the same
