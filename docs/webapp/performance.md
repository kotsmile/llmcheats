---
title: Performance — database, deadlines, concurrency, caching
summary: Bounded connection pools and set-based queries, a justified HTTP deadline budget hierarchy, errgroup and singleflight for concurrency, and TTLs derived from upstream expiry.
keywords: [connection pool, index, N+1, SKIP LOCKED, work queue, deadline budget, timeout, WriteTimeout, ReadHeaderTimeout, streaming, errgroup, singleflight, WithoutCancel, cache, TTL, staleTime, lazy load, virtualize]
related:
...
# Performance — database, deadlines, concurrency, caching
...
connections; the `BEGIN` retry in `webapp/backend-infrastructure.md` absorbs
the recycling moment.

**Indexes**: composite, ordered to match the query — a feed reading
`order by at desc, id desc` gets `(resource, at desc, id desc)` (the tiebreaker
column belongs in the index too); a uniqueness rule gets a unique index that
also backs the upsert. Every index has a written justification; FK columns you
join on get one (Postgres will not).

**Caps everywhere**: every list endpoint clamps `limit` server-side.

## Avoiding N+1 queries

Set-based queries are the default:

- "Everything this subject can see" is **one** query with `subject = any($1)`
  over the expanded subject list — never a query per group.
...
- "Least-loaded assignee" is one `LEFT JOIN … GROUP BY … ORDER BY COUNT` — not
  a fan-out.

## Claiming work from a Postgres queue

`FOR UPDATE SKIP LOCKED` for multi-replica job claiming — every replica ticks,
the claim query stops duplicate processing. One job per claim keeps lock scopes
small.

## Budgeting HTTP deadlines
...
per-request deadlines too; "internal" is not a deadline exemption when it calls
cloud APIs that hang.

## Managing concurrency

- **errgroup as process supervisor** (`webapp/backend-config-lifecycle.md`) and
  as a fan-out primitive for parallel independent work (calling three
...
- Never a bare `go func()` in request paths: panic-recovering spawn helper or a
  bounded pool that reports "busy".
...
- **Derive TTLs from upstream expiry**: cache a signed URL for **half** its
  ...
  remaining validity is under a threshold as a miss ("never hand out a URL that
  may die mid-use"). Emit cache refresh reasons (`miss` / `near_expiry`) as
  metric labels.
- **Do not cache**: high-cardinality cheap queries (search results), and
  **never** revealed secrets — on either side of the wire.
- Frontend caching is TanStack Query's `staleTime` tiers
  (`webapp/frontend-data.md`).
...
- No hand memoization (`webapp/frontend-react.md`); if lists get large,
